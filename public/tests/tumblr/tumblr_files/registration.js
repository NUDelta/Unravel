...
(function (c, b) {
  var a = Backbone.View.extend({
    el: "body", initialize: function () {
      ...
      if (!c("#illustrated_loader").length) {
      ...
      if (this.animation_method == "interval") {
        ...
        var d = new Date().getTime(), e = d - (this.animation_time || d);
      ...
      this.$loader.css({"background-position": "-" + (this.current_frame * this.frame_width) + "px 0px"})
    }
  });
...
Tumblr.RegistrationForm = (function (F) {
  var I, ad, B, u, q, p, l, c, v, W, Z, j, t = false, g = [], S, D, R, b, U, s, z, A, J, o, aa;
  var y = (document.location.protocol === "https:"), H = "";
  ...
  function i(af) {
    ...
    if (X(af)) {
      ...
      if (F("body").hasClass("has_login_captcha")) {
        ...
        if (F("body").hasClass("has_login_tfa")) {
          ...
          F("#tfa_sms_resend").click(function (am) {
            am.preventDefault();
            ac()
          })
        } else {
    ...
    if (af == "signup_account") {
      ...
      if (Tumblr.Flags.bool("show_random_username_suggestions")) {
        var aj = JSON.parse(F("#random_username_suggestions").val());
        if (aj) {
          F("#signup_username").one("focus", function () {
    ...
    if (af == "signup_waiting") {
      if (A) {
        ...
        var al = new Spinner(l.spinner_opts).spin();
        F("#signup_waiting").append(F(al.el))
      }
      if (F("#failed_redirect_link").length) {
        F("#failed_redirect_link").addClass("show")
      }
      F(window).off("keydown keyup keypress")
    }
  }
  ...
  function ac() {
    ...
    o.resend_token_sms(F("#signup_email").val(), F("#tfa_form_key"))
  }

  function ab() {
    F("#" + l.current_view + " input").each(function (ae) {
  ...
  function n(ai, ae) {
    ...
    if (ae === true) {
      ...
      F(".signup_username").one("keypress", function () {
        F(".signup_username_checkmark").hide()
      })
    }
    aj.html("");
      ...
      if (an.attr("id") == "signup_email" && !ag.match(/\@ymail.com$/i)) {
        ...
        if (!/\@/.test(ag)) {
          ...
          if (ag.toLowerCase().indexOf("@tumblr.com") != -1) {
            ...
            if (!t && (ai = Tumblr.SpellChecker.suggest(ak)).length) {
              aq = ag.split("@")[0] + "@" + ai[0];
              if (aq != ag) {
  ...
  function E() {
    ...
    if (X(l.current_view)) {
      ...
      if (l.current_view === "signup_account" && !p.find("#signup_username").val()) {
          ...
          F.ajax(p.attr("action"), {
            type: "POST", data: signup_form_data, error: function (ae, ah, ag) {
              ...
              if (ae.redirect) {
                ...
                d(F("#" + l.current_view).next().attr("id"))
              }
            }
          })
        }
  ...
  function w() {
    var ae = document.activeElement, af = (u) ? u.length - 1 : false;
    if (ae && af && u[af]) {
  ...
  function e() {
    if (F("body").hasClass("signup_login")) {
      ...
      if (F("body").hasClass("signup_account")) {
        ...
        if (F("body").hasClass("signup_waiting")) {
  ...
  return {
    initialize: function (ae) {
      ...
      D = (R || b);
      s = (F(document.body).hasClass("show_password_strength"));
      p = F("#signup_form");
