...
var registerShoveller = function(b) {
    b.fn.s9Shoveler = function(d) {
        var c = {
            translations: {
                next: "next",
                back: "back",
                of: "of",
                pageNOfM: "Page %1$s of %2$s"
            },
            minItems: 3,
            maxShovelerItems: 20,
            seeded: false,
            ...
            logClicks: false,
            logSlotName: "",
            fadeShovelerControls: false,
            emulateOldShoveler: true,
            onPageChanged: null,
    ...
    function a(g, x) {
        ...
        if (!t) {
            throw ("No multipack found on " + g[0].id)
        }
        var w = g.hasClass("aui-showgrid");
        var G = 1;
        var D = t.getColumnCount();
        var s = 0;
        if (x.emulateOldShoveler && x.fadeShovelerControls) {
            x.fadeShovelerControls = false
        }
        var i = b(".s9OtherItems", g);
        var I = g.parent(".unified_widget");
        I.addClass("s9Shoveler");
        if (x.emulateOldShoveler) {
            I.addClass("s9BookendShoveler")
        }
        if (x.seeded) {
            var j = b(".s9SeedItem", g);
        ...
        O();
        var l = i.parents(".unified_widget, .widget")
            .children("h2");
        var k;
        var n, v, C, e, A, T, U, H;
        y();
        if (x.fadeShovelerControls) {
            m()
        }
        ...
        function p(ac, ab, Y) {
            ...
            if (ab) {
                var Z = function(ae) {
                    if (ab == o) {
                        return (R.length - ae - 1) * x.cellChangeSpeedInMs
                    } else {
                        return ae * x.cellChangeSpeedInMs
                    }
                };
                var aa = R.length;
                R.each(function(ae) {
        ...
        function u(W, X) {
            D = Math.max(x.minItems, Math.min(x.maxShovelerItems, X.potentialCols));
            V();
            p(Math.min(G, s));
            h();
            setTimeout(function() {
        ...
        function V() {
            ...
            if (X > 0) {
                for (var aa = 0; aa < X; aa++) {
                    i.append('<div class="s9ShovelerCell"/>')
                }
            } else {
                if (X < 0) {
                    var W = R.slice(R.length + X, R.length);
                    J.append(W.children());
                    W.remove()
                }
            }
            ...
            if (B) {
                R.css("margin-right", "")
                    .filter(":last")
                    .css("margin-right", "-" + (D) + "px");
                if (x.seeded) {
                    i.css("margin-right", "-6px")
                }
            }
            E()
        }
        ...
        function M(X) {
            ...
            var Z = 0;
            if (x.seeded) {
                Z = d[0].offsetHeight + L(d.css("margin-bottom"), 0)
            }
            if (ac && X === true || ac > L(i.css("height"), 0) - Z) {
                i.css("height", (ac + Z) + "px")
            }
        }
        ...
        function y() {
            if (!x.emulateOldShoveler) {
                var W = l.children(".s9ShovelerPaging");
                if (!W.length) {
                    F();
                    ...
                    H = b('<div class="s9ShovelerPaging"/>')
                        .append(e)
                        .append(C)
                        .append(U)
                        .append(n)
                    ...
                    l.addClass("s9ShovelerHeader s9ShovelerButtonHeader")
                        .prepend(H);
                    K(v, n);
                    K(e, C)
                } else {
                    n = W.find(".s9ShovelerNext");
        ...
        function S(X) {
            if (k) {
                ...
                if (q) {
                    g.trigger("s9MultipackResize")
                }
            }
        }

        function F(W) {
            A = b('<span class="s9ShovelerCurrent"/>')
                .text("1");
            T = b('<span class="s9ShovelerTotal"/>');
            U = b('<span class="s9ShovelerPage' + (W ? " " + W : "") + '"/>');
            if (!x.emulateOldShoveler) {
                U.append(A)
                    .append(" " + x.translations.of + " ")
                    .append(T)
            } else {
                var Y = x.translations.pageNOfM;
                var X = Y.match(/(.*)%(\d)\$s(.*)%(\d)\$s(.*)/);
                if (!X) {
                    throw "S9 Shoveler translation substitution failed"
                }
                U.append(X[1]);
                U.append(X[2] == "1" ? A : T);
                U.append(X[3]);
        ...
        function O() {
            J.append(Q);
            Q.each(function() {
                this.className = this.className.replace(/ s9a(\d+)/, "")
            });
            Q.find(".imageContainer img")
                .each(function() {
                    var X = b(this);
                    var W = X.attr("style");
                    if (W) {
                        var Y = W.match(/background-image\s*:\s*url\(\s*["']?([^'";)]+)["']?\)/i);
                        if (Y && Y[1]) {
                            X.attr("url", Y[1]);
        ...
        function r(W) {
            if (x.logClicks && window.clientLogger) {
                setTimeout(function() {
                    clientLogger.sendCLOGEntry("s9", "shov2", {
                        direction: (W == o ? "next" : "back"),
                        slot: x.logSlotName
                    })
                })
            }
    ...
    if (window.amznJQ !== undefined) {
        amznJQ.declareAvailable("s9Shoveler")
    }
};
if (window.amznJQ !== undefined) {
    amznJQ.onReady("jQuery", function() {
        registerShoveller(jQuery)
    })
} else {
    if (window.P !== undefined) {
        P.when("A", "ready")
            .register("s9Shoveler", function(a) {
                registerShoveller(a.$)
            })
    }
};