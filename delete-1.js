// Don't remove this line: Begin Source File: /javascripts/lib/jquery-2.1.4.js
/*!
...
!function(global, factory) {
  if ("object" === typeof module && "object" === typeof module.exports) // For CommonJS and CommonJS-like environments where a proper `window`
 ...
 */
    setDocument = Sizzle.setDocument = function(node) {
      ...
      if (parent.addEventListener) parent.addEventListener("unload", unloadHandler, false); else if (parent.attachEvent) parent.attachEvent("onunload", unloadHandler);
      /* Support tests
	---------------------------------------------------------------------- */
 ...
 */
    getText = Sizzle.getText = function(elem) {
      ...
      ret += getText(node); else if (1 === nodeType || 9 === nodeType || 11 === nodeType) // Use textContent for elements
      // innerText usage removed for consistency of new lines (jQuery #11153)
      if ("string" === typeof elem.textContent) return elem.textContent; else // Traverse its children
        ...
        CHILD: function(type, what, argument, first, last) {
          ...
          return 1 === first && 0 === last ? function(elem) {
            ...
            if (parent) {
              ...
              while (node = ++nodeIndex && node && node[dir] || (diff = nodeIndex = 0) || start.pop()) if ((ofType ? node.nodeName.toLowerCase() === name : 1 === node.nodeType) && ++diff) {
    ...
    function addCombinator(matcher, combinator, base) {
      ...
      return combinator.first ? function(elem, context, xml) {
        ...
        if (xml) {
          ...
          if ((oldCache = outerCache[dir]) && oldCache[0] === dirruns && oldCache[1] === doneName) // Assign to newCache so results back-propagate to previous elements
  ...
  rquickExpr = /^(?:\s*(<[\w\W]+>)[^>]*|#([\w-]*))$/, init = jQuery.fn.init = function(selector, context) {
    ...
    if ("string" === typeof selector) {
      ...
      if (match[1]) {
        ...
        if (jQuery.isFunction(this[match])) this[match](context[match]); else this.attr(match, context[match]);
        return this;
      } else {
      ...
      this.length = 1;
      return this;
    } else if (jQuery.isFunction(selector)) // Execute immediately if ready is not present
        ...
        if (list) {
          ...
          !function add(args) {
            jQuery.each(args, function(_, arg) {
              ...
              } else if (arg && arg.length && "string" !== type) // Inspect recursively
              add(arg);
            });
          }(arguments);
 ...
 */
  ...
  jQuery.ready.promise = function(obj) {
    if (!readyList) {
      ...
      if ("complete" === document.readyState) // Handle it asynchronously to allow scripts the opportunity to delay ready
  ...
  Data.prototype = {
    ...
    access: function(owner, key, value) {
      ...
      if (void 0 === key || key && "string" === typeof key && void 0 === value) {
        ...
        return void 0 !== stored ? stored : this.get(owner, jQuery.camelCase(key));
      }
    ...
    remove: function(owner, key) {
      ...
      if (void 0 === key) this.cache[unlock] = {}; else {
        ...
        name = key.concat(key.map(jQuery.camelCase)); else {
    ...
    data: function(key, value) {
      ...
      return access(this, function(value) {
        ...
        if (elem && void 0 === value) {
          ...
          data = dataAttr(elem, camelKey, void 0);
          if (void 0 !== data) return data;
        ...
        this.each(function() {
          ...
          // unchanged property.
          if (key.indexOf("-") !== -1 && void 0 !== data) data_user.set(this, key, value);
        });
 ...
 */
  jQuery.event = {
    ...
    trigger: function(event, data, elem, onlyHandlers) {
      ...
      if (!onlyHandlers && !event.isDefaultPrevented()) if ((!special._default || false === special._default.apply(eventPath.pop(), data)) && jQuery.acceptData(elem)) // Call a native DOM method on the target with the same name name as the event.
    ...
    handlers: function(event, handlers) {
      ...
      if (true !== cur.disabled || "click" !== event.type) {
        matches = [];
  ...
  jQuery.Event = function(src, props) {
    ...
    if (src && src.type) {
      ...
      this.isDefaultPrevented = src.defaultPrevented || void 0 === src.defaultPrevented && // Support: Android<4.0
  ...
  jQuery.extend({
    ...
    buildFragment: function(elems, context, scripts, selection) {
      ...
      for (;i < l; i++) {
        elem = elems[i];
        if (elem || 0 === elem) // Add nodes directly
        if ("object" === jQuery.type(elem)) // Support: QtWebKit, PhantomJS
        // push.apply(_, arraylike) throws on ancient WebKit
        jQuery.merge(nodes, elem.nodeType ? [ elem ] : elem); else if (!rhtml.test(elem)) nodes.push(context.createTextNode(elem)); else {
      ...
      for (;void 0 !== (elem = elems[i]); i++) {
        if (jQuery.acceptData(elem)) {
          ...
          if (key && (data = data_priv.cache[key])) {
            ...
            if (data_priv.cache[key]) // Discard any remaining `private` data
        ...
        if (first) {
          ...
          if (hasScripts) {
            ...
            for (i = 0; i < hasScripts; i++) {
              node = scripts[i];
              if (rscriptType.test(node.type || "") && !data_priv.access(node, "globalEval") && jQuery.contains(doc, node)) if (node.src) {
 ...
 */
  ...
  !function() {
    ...
    if (window.getComputedStyle) jQuery.extend(support, {
      ...
      boxSizingReliable: function() {
        if (null == boxSizingReliableVal) computePixelPositionAndBoxSizingReliable();
  ...
  function getWidthOrHeight(elem, name, extra) {
    ...
    var valueIsBorderBox = true, val = "width" === name ? elem.offsetWidth : elem.offsetHeight, styles = getStyles(elem), isBorderBox = "border-box" === jQuery.css(elem, "boxSizing", false, styles);
  ...
  function showHide(elements, show) {
    ...
    for (;index < length; index++) {
      ...
      if (show) {
        ...
        if ("none" !== display || !hidden) data_priv.set(elem, "olddisplay", hidden ? display : jQuery.css(elem, "display"));
      }
    }
  ...
  jQuery.extend({
    ...
    style: function(elem, name, value, extra) {
      ...
      if (void 0 !== value) {
        ...
        if (!hooks || !("set" in hooks) || void 0 !== (value = hooks.set(elem, value, extra))) style[name] = value;
      } else {
  ...
  function createTween(value, prop, animation) {
    ...
    for (;index < length; index++) if (tween = collection[index].call(animation, prop, value)) // We're done with this property
    return tween;
  }
  function defaultPrefilter(elem, props, opts) {
    ...
    if (1 === elem.nodeType && ("height" in props || "width" in props)) {
      ...
      if ("inline" === checkDisplay && "none" === jQuery.css(elem, "float")) style.display = "inline-block";
    }
    if (opts.overflow) {
    ...
    for (prop in props) {
      ...
      if (rfxtypes.exec(value)) {
        ...
        if ("show" === value && dataShow && void 0 !== dataShow[prop]) hidden = true; else continue;
    ...
    } else if ("inline" === ("none" === display ? defaultDisplay(elem.nodeName) : display)) style.display = display;
  }
  ...
  boolHook = {
    set: function(elem, value, name) {
      if (false === value) // Remove boolean attributes when set to false
      jQuery.removeAttr(elem, name); else elem.setAttribute(name, name);
      return name;
    }
  };
    ...
    toggleClass: function(value, stateVal) {
      ...
      return this.each(function() {
        if ("string" === type) {
          ...
          if (this.className) // store className if set
 ...
 */
  function ajaxConvert(s, response, jqXHR, isSuccess) {
    ...
    while (current) {
      ...
      if ("*" === current) current = prev; else if ("*" !== prev && prev !== current) {
        ...
        if (true !== conv) // Unless errors are allowed to bubble, catch and return them
  ...
  jQuery.extend({
    ...
    ajax: function(url, options) {
      ...
      if (!s.hasContent) {
        ...
        if (false === s.cache) s.url = rts.test(cacheURL) ? // If there is already a '_' parameter, set its value
      ...
      if (s.beforeSend && (false === s.beforeSend.call(callbackContext, jqXHR, s) || 2 === state)) // Abort if not done already and return
      return jqXHR.abort();
  ...
  function buildParams(prefix, obj, traditional, add) {
    ...
    }); else if (!traditional && "object" === jQuery.type(obj)) // Serialize object item.
        ...
        callback = function(type) {
          return function() {
            if (callback) {
              ...
              if ("abort" === type) xhr.abort(); else if ("error" === type) complete(// file: protocol always yields status 0; see #8605, #14207
 ...
 */
  ...
  jQuery.fn.extend({
    ...
    position: function() {
      ...
      if ("fixed" === jQuery.css(elem, "position")) // Assume getBoundingClientRect is there when computed position is fixed
  ...
  return jQuery;
});// Don't remove this line: End Source File: /javascripts/lib/jquery-2.1.4.js


// Don't remove this line: Begin Source File: /-script-3
var bazz = function() {
  var i = 0;
  i++;
  bar();
  return i;
};// Don't remove this line: End Source File: /-script-3


// Don't remove this line: Begin Source File: /javascripts/home.js
...
var bar = function() {
  var $effect1 = $("#effect1");
  if ($effect1.is(":visible")) $effect1.hide(); else $effect1.show();
};
...
var fourthFunction = function(triggerData) {
  $effect3.show("fast");
  window.setTimeout(eighthFunction, 1e3);
};
...
var sixthFunction = function() {
  ...
  window.setTimeout(eightFunction, 1e3);
};

var eighthFunction = function() {
  $effect3.hide("fast");
};// Don't remove this line: End Source File: /javascripts/home.js
